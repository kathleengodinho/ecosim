...
void simulatePlant(int i, int j)
{
    ...
    while (true)
    {
        ...
        if (getProbability(PLANT_REPRODUCTION_PROBABILITY))
        {

            if (i + 1 < NUM_ROWS && entity_grid[i + 1][j].type == empty)
            {
                std::cout << "New plant i " << j + 1 << " j " << j << std::endl;
...
int main()
{
    ...
    CROW_ROUTE(app, "/next-iteration")
        ...
        while(*iteracao_terminada < total_entidades) {
            std::cout << "I_t " << iteracao_terminada << " T_e " << total_entidades << std::endl;
            sleep(1);
        }
